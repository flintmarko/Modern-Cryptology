...
/*INT sw1;   */
/*type of cryption 0= encryption, 1= decryption */
/* BYTE * pkey;
  INT r; */
{
	 INT i, j, k, t1, t2;
	int z;
	// for (i=0; i < 56; i++) {   // no need for PC-1
	//   CD[i] = key[PC1[i] -1];
	// printf("%d", CD[i]);
	// }
...
INT RFP[] = { 8, 40, 16, 48, 24, 56, 32, 64,
...
int main()
{
	...
	ifile1.open("key_output.txt");
	ofile1.open("key_out.txt");
	while (getline(ifile1, str1))
	{
		...
		string s = "gsklmhtounorfuiu";
		BYTE cipher1[9];	//rnlusogrkiqrkkqo
		for (int i = 0; i < 8; i++)
		{
			int x = s[2 *i] - 'f';
			int y = s[2 *i + 1] - 'f';
