...
void set_the_key(int sw1,BYTE *CD,int r)  // we are directly feeding CD as input which hence no need for PC-1
     ...
     INT r;*/
{
   INT i, j, k, t1, t2;
  int z;
 // for (i=0; i<56; i++) { // PC-1 not needed 
 //   CD[i] = key[PC1[i] -1];
    // printf("%d", CD[i]);
 // }
...
int main()
{
    ...
    ifstream ifile1;
    ofstream ofile1;
    ofile1.open("password.txt");
    sw1=1;
    string s = "01101110010111100111101110001110101100011000001001110001"; // decrypted key from key scheduling 
                                                                           // it is CD value after PC-1 
    BYTE key[56]; 
    for(int i=0;i<56;i++){    // storing the key in key array 
      key[i] = s[i]-'0';
    }
    r=6;
    BYTE plain[9];    
     BYTE plain2[9];               
    string p = "iikiqgmnlgqrnjjf";
    string q = "pjqmrfrjkggnrgnt";
    // iikiqgmnlgqrnjjfpjqmrfrjkggnrgnt
    for(int i=0;i<8;i++){      // converting block to byte values 
      int x  = p[2*i]-'f';
      int y = p[2*i+1]-'f';
      plain[i] = 16*x+y;
    }
    ...
    des(plain,cipher2,r,flag);
     des(plain2,cipher3,r,flag);
    for(i=0;i<8;i++){
    ofile1<<char(cipher2[i]-'a'+'a');    //   111 118 116 115 104 117 100 102 109 97 48 48 48 48 48 48 
    }
    ofile1<<"\n";
    for(i=0;i<8;i++){
    ofile1<<int(cipher2[i])<<" ";
    }
    ofile1<<"\n";
     for(i=0;i<8;i++){
    ofile1<<char(cipher3[i]-'a'+'a');    //   111 118 116 115 104 117 100 102 109 97 48 48 48 48 48 48 
    }
    ofile1<<"\n";
    for(i=0;i<8;i++){
    ofile1<<int(cipher3[i])<<" ";
    }
    return 0;
}